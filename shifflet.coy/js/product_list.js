...
$(() => {
	...
	$(".js-sort").on("change", function(e) {
		query(
			this.value == 1 ? {type: 'product_sort', column: 'date_create', dir: 'DESC'} :
			this.value == 2 ? {type: 'product_sort', column: 'date_create', dir: 'ASC'} :
			this.value == 3 ? {type: 'product_sort', column: 'price', dir: 'ASC'} :
			this.value == 4 ? {type: 'product_sort', column: 'price', dir: 'DESC'} :
			{type: 'products_all'}
		).then(showResults);
	});
});
